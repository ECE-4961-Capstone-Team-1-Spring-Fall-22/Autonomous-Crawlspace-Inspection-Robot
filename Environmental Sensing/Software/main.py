...
#     Images = ReadImage("TestPics")
#     StitchedImage = Images[0]
#     for i in range(6, len(Images)):#start from 1 and go to last in array
#         MiddleStitch = StitchImages(Images[i-1], Images[i])
#         print(i)
#         StitchedImage = StitchImages(StitchedImage, MiddleStitch)
#         print(i)
#
#     plt.axis('off')
...
#     Images = ReadImage("TestPics")
#     StitchedImage = Images[0]
#     for i in range(0, len(Images)):#start from 1 and go to last in array
#         StitchedImage = StitchImages(StitchedImage, Images[i])
#         print(i)
#
#     plt.axis('off')
#     plt.imshow(StitchedImage)
...
if __name__ == "__main__":
    ...
    temps = np.loadtxt('temp.txt', dtype=float)
    hums = np.loadtxt('hum.txt', dtype=float)
    mcs = np.loadtxt('mc.txt', dtype=float)
    ...
    for i in range(len(temps)):
        if temps[i] > avgTemp+(3*stdTemp) or temps[i] < avgTemp-(3*stdTemp):
            Images[i] = cv2.putText(Images[i], str(temps[i]), (450, 620), cv2.FONT_HERSHEY_SIMPLEX,
                                2, (0, 0, 200), 2, cv2.LINE_AA)
        else:
            continue
    for i in range(len(hums)):
        if hums[i] > avgHum+(3*stdHum) or hums[i] < avgHum-(3*stdHum):
            Images[i] = cv2.putText(Images[i], str(hums[i]), (450, 220), cv2.FONT_HERSHEY_SIMPLEX,
                                2, (0, 200, 0), 2, cv2.LINE_AA)
        else:
            continue

    #mcs = [7, 8, 9, 11, 10, 8, 13, 15, 18, 10]
    for i in range(len(mcs)):
        Images[i] = cv2.putText(Images[i], str(mcs[i]), (450, 420), cv2.FONT_HERSHEY_SIMPLEX,
                                3, (0, 0, 0), 2, cv2.LINE_AA)

    #Calling function for stitching images.
    StitchedImage1 = StitchImages(Images[0], Images[1])
    print(1)
    StitchedImage2 = StitchImages(Images[2], Images[3])
    print(1)
    StitchedImage3 = StitchImages(Images[5], Images[6])
    print(1)
    StitchedImage4 = StitchImages(Images[7], Images[8])
    print(1)
    print(1)

    Stitch1 = StitchImages(StitchedImage1, StitchedImage2)
    print(2)
    Stitch2 = StitchImages(StitchedImage3, StitchedImage4)
    print(2)
    #Stitch3 = StitchImages(StitchedImage4, StitchedImage5)
    print(2)

    Stitch1R = cv2.rotate(Stitch1, cv2.ROTATE_90_CLOCKWISE)

    Final = StitchImages(Stitch1R, Stitch2)
